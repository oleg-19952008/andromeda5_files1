	...
	public class PureUdpClient
	{
		...
		private byte[] ReadFromStream(NetworkStream ns, int len)
		{
			...
			while (num < len)
			{
				...
				if (ns.DataAvailable)
				{
					num = num + ns.Read(numArray, num, len - num);
				...
				while (this.isRunning)
				{
					...
					else
					{
						...
						for (int i = 0; i < (int)udpCommHeaderArray.Length; i++)
						{
							...
							int bytes = udpCommHeader.ToBytes(this.mySecretGun.bufferOut, 0) + 4;
		...
		public void StartReceiveUdp(Action<Exception, PlayerData> onError)
		{
			...
			{
				bufferIn = new byte[4000000],
				bufferOut = new byte[1000000],
				onIoError = onError
			};
			Thread thread = new Thread(new ThreadStart(this.RunReceiver));
			this.isRunning = true;
			thread.Start();
		}
