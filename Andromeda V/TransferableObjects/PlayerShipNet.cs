...
public class PlayerShipNet : ITransferable
{
	...
	public void ApplyAttachmentBonus(ushort itemType)
	{
		...
		if (PlayerItems.item2categoryMapping[itemType] == 4)
		{
			...
			PlayerShipNet shieldHP = this;
			shieldHP.ShieldHP = shieldHP.ShieldHP + item.bonusValue;
		}
		if (PlayerItems.item2categoryMapping[itemType] == 6)
		{
			item = (GeneratorNet)StaticData.allTypes[itemType];
			PlayerShipNet corpusHP = this;
			corpusHP.CorpusHP = corpusHP.CorpusHP + item.bonusValue;
		}
	}
	...
	public void CalculateAdditionalBonuses(PlayerBelongings pb)
	{
		...
		for (int i = 0; i < (int)slotItemArray.Length; i++)
		{
			...
			{
				bonusFive = bonusFive + (float)slotItem.BonusFive;
			}
			if (PlayerItems.IsForShieldRegen(slotItem.ItemType))
			{
				bonusFive = bonusFive + (float)((ExtrasNet)StaticData.allTypes[slotItem.ItemType]).efValue;
			}
			...
			if (PlayerItems.IsExtraDamage(slotItem.ItemType))
			{
				...
				bonusFive1 = bonusFive1 + slotItem.BonusFour;
				powerUpEffectValue1 = powerUpEffectValue1 + slotItem.BonusFive;
			}
		...
		{
			bonusFour = bonusFour + pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForLaserDamageFlat);
		}
		if (pb.HaveLaserDamagePercentage)
		{
			single1 = single1 + (float)pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForLaserDamagePercentage);
		}
		if (pb.HavePlasmaDamageFlat)
		{
			bonusFive1 = bonusFive1 + pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForPlasmaDamageFlat);
		}
		if (pb.HavePlasmaDamagePercentage)
		{
			powerUpEffectValue2 = powerUpEffectValue2 + (float)pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForPlasmaDamagePercentage);
		}
		if (pb.HaveIonDamageFlat)
		{
			powerUpEffectValue1 = powerUpEffectValue1 + pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForIonDamageFlat);
		}
		if (pb.HaveIonDamagePercentage)
		{
			single2 = single2 + (float)pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForIonDamagePercentage);
		}
		if (pb.HaveTotalDamagePercentage)
		{
			powerUpEffectValue = powerUpEffectValue + (float)pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForTotalDamagePercentage);
		}
		pb.playerItems.GetSkillEffect(PlayerItems.TypeTalentsEmpoweredShield, out num, out num1);
		bonusFive = bonusFive + (float)(num + num1);
	...
	private short CalculateAvoidanceBonus(ShipsTypeNet shipType, PlayerBelongings pb)
	{
		...
		for (int i = 0; i < (int)slotItemArray.Length; i++)
		{
			...
			{
				bonusFour = bonusFour + (double)slotItem.BonusFour;
			}
			...
			{
				bonusFour = bonusFour + (double)slotItem.BonusTwo;
			}
			else if (PlayerItems.IsExtraOther(slotItem.ItemType))
			{
				bonusFour = bonusFour + (double)slotItem.BonusThree;
			}
		}
		bonusFour = bonusFour + (double)((float)(shipType.avoidance * this.guildAccuracy) / 100f);
		pb.playerItems.GetSkillEffect(PlayerItems.TypeTalentsNanoTechnology, out num, out num1);
		bonusFour = bonusFour + (double)(num + num1);
		pb.playerItems.GetSkillEffect(PlayerItems.TypeTalentsDefiance, out num, out num1);
		bonusFour = bonusFour + (double)((float)(shipType.avoidance * (num + num1)) / 100f);
		if (pb.HaveAvoidanceFlat)
		{
			bonusFour = bonusFour + (double)pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForAvoidanceFlat);
		}
		if (pb.HaveAvoidancePercentage)
		{
			powerUpEffectValue = powerUpEffectValue + (float)pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForAvoidancePercentage);
		}
	...
	private short CalculateCargoBonus(ShipsTypeNet shipType, PlayerBelongings pb)
	{
		...
		for (int i = 0; i < (int)slotItemArray.Length; i++)
		{
			...
			{
				item = item + (float)((ExtrasNet)StaticData.allTypes[slotItem.ItemType]).efValue;
			}
			if ((PlayerItems.IsEngine(slotItem.ItemType) ? true : PlayerItems.IsExtraCargoMining(slotItem.ItemType)))
			{
				bonusThree = bonusThree + (double)slotItem.BonusThree;
				item = item + (float)slotItem.BonusFour;
			}
		}
		...
		bonusThree = bonusThree + (double)((float)shipType.cargo * ((float)(num + num1) + item) / 100f);
		if (pb.HaveCargoBooster)
		{
			bonusThree = bonusThree + (double)shipType.cargo;
		}
	...
	private int CalculateCorpusBonus(ShipsTypeNet shipType, PlayerBelongings pb)
	{
		...
		for (int i = 0; i < (int)slotItemArray.Length; i++)
		{
			...
			{
				item = item + (double)((GeneratorNet)StaticData.allTypes[slotItem.ItemType]).bonusValue;
				item = item + (double)slotItem.BonusOne;
				bonusTwo = bonusTwo + (float)slotItem.BonusTwo;
			}
		}
		bonusTwo = bonusTwo + (float)this.guildEndurance;
		if (pb.HaveCorpusFlat)
		{
			item = item + (double)pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForCorpusFlat);
		}
		if (pb.HaveCorpusPercentage)
		{
			bonusTwo = bonusTwo + (float)pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForCorpusPercentage);
		}
		if (pb.HaveEndurancePercentage)
		{
			bonusTwo = bonusTwo + (float)pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForEndurancePercentage);
		}
		item = item + (double)((float)shipType.corpus * bonusTwo / 100f);
	...
	private int CalculateShieldBonus(ShipsTypeNet shipType, PlayerBelongings pb)
	{
		...
		bonusTwo = bonusTwo + (float)(num + num1);
		bonusTwo = bonusTwo + (float)this.guildEndurance;
		if (pb.HaveShieldFlat)
		{
			item = item + (double)pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForShieldFlat);
		}
		if (pb.HaveShieldPercentage)
		{
			bonusTwo = bonusTwo + (float)pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForShieldPercentage);
		}
		if (pb.HaveEndurancePercentage)
		{
			bonusTwo = bonusTwo + (float)pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForEndurancePercentage);
		}
		item = item + (double)((float)shipType.shield * bonusTwo / 100f);
	...
	private short CalculateTargetingBonus(ShipsTypeNet shipType, PlayerBelongings pb)
	{
		...
		for (int i = 0; i < (int)slotItemArray.Length; i++)
		{
			...
			{
				bonusThree = bonusThree + (double)slotItem.BonusOne;
			}
			else if (PlayerItems.IsExtraOther(slotItem.ItemType))
			{
				bonusThree = bonusThree + (double)slotItem.BonusTwo;
			}
		}
		bonusThree = bonusThree + (double)((float)(shipType.targeting * this.guildAccuracy) / 100f);
		pb.playerItems.GetSkillEffect(PlayerItems.TypeTalentsSteadyAim, out num, out num1);
		bonusThree = bonusThree + (double)(num + num1);
		pb.playerItems.GetSkillEffect(PlayerItems.TypeTalentsFindWeakSpot, out num, out num1);
		bonusThree = bonusThree + (double)(num + num1);
		if (pb.HaveTargetingFlat)
		{
			bonusThree = bonusThree + (double)pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForTargetingFlat);
		}
		if (pb.HaveTargetingPercentage)
		{
			powerUpEffectValue = powerUpEffectValue + (float)pb.GetPowerUpEffectValue(PlayerItems.TypePowerUpForTargetingPercentage);
		}
